...
- `VERSION` contiene la versión SemVer instalada de `tech-cleanup` en formato `MAJOR.MINOR.PATCH`.
- El tag Git publicado usa `tech-cleanup-vMAJOR.MINOR.PATCH` para evitar colisiones con otras skills del repositorio.
...
- `MAJOR`: cambia de forma incompatible el contrato de auditoría, sus garantías o formatos de salida.
- `MINOR`: agrega un modo, capacidad, regla, referencia o script compatible.
- `PATCH`: corrige o aclara comportamiento sin agregar capacidades ni romper compatibilidad.

Todo cambio persistente dentro de `skills/tech-cleanup/` debe incluir en la misma Pull Request:
