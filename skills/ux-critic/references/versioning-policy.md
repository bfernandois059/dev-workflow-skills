...
- `VERSION` contiene la versión SemVer instalada de `ux-critic` en formato `MAJOR.MINOR.PATCH`.
- El tag Git publicado usa `ux-critic-vMAJOR.MINOR.PATCH` para evitar colisiones con otras skills del repositorio.
...
- `MAJOR`: cambia de forma incompatible el contrato de la crítica, sus capas de juicio o los formatos de salida.
- `MINOR`: agrega una capa, prueba, regla, referencia o script compatible.
- `PATCH`: corrige o aclara comportamiento sin agregar capacidades ni romper compatibilidad.

Todo cambio persistente dentro de `skills/ux-critic/` debe incluir en la misma Pull Request:
